...
// function makeFour() {
// 	const paragraphFour = document.getElementById('practicum4');
// 	const one = document.forms.formOne.getAttribute("name");
// 	const two = document.forms.formTwo.getAttribute("name");
// 	const three = document.forms.formThree.getAttribute("name");
// 	const four = document.forms.lastForm.getAttribute("name");
// 	const formNames = [one, two, three, four];
// 	paragraphFour.innerHTML = formNames.join(',');
// }
...
function makeFive() {
	...
	paragraphFive.innerHTML = formThree.elements.length
}
...
function makeSix() {
	...
	paragraphSix.innerHTML = formTwo.elements.length
}
...
function makeEight() {
	...
	const formFirstElements = Array.from(document.forms[0].elements);
	let elementsList = '';
	for (let i = 0; i < formFirstElements.length; i++) {
		elementsList = elementsList + formFirstElements[i].getAttribute('name') + ', ';
	}
...
function makeNine() {
	...
	const formFirstElements = Array.from(document.forms[2].elements);
	let elementsList = '';
	for (let i = 0; i < formFirstElements.length; i++) {
		elementsList = elementsList + formFirstElements[i].getAttribute('name') + ', ';
	}
...
function makeTen() {
	...
	paragraphTen.innerText = formFirstElements[0].value
}
...
function makeTwelve() {
	...
	// let checkBoxIds = '';
	// for (let i = 0; i < firstFormElements.length; i++) {
	// 	if (firstFormElements[i].classList.value === 'form__checkbox')
	// 		checkBoxIds = checkBoxIds + firstFormElements[i].getAttribute('id') + ', ';
	// }
	paragraphTwelve.innerHTML = firstFormElements[2].id + firstFormElements[3].id + firstFormElements[4].id;
}
...
function checkButton(e) {
	...
	else paragraphThirteen.innerHTML = "Кнопка no выбрана";
}
...
function checkOption() {
	...
	options.forEach((option) => {
		if (option.selected) paragraphFourteen.innerHTML = option.value
	})
}
...
function makeSixteen() {
	const checkbox3 = document.forms[1].elements[4];
...
// 	const formOneElements = Array.from(document.forms.formOne.elements);
// 	formOneElements.forEach((element) => {
// 		if (!element.value && element.name !== 'firstButton') {
// 			document.getElementById('errorMessage').innerHTML = 'Пусто';
// 			return;
// 		}
// 	})
// });
...
emailInput.addEventListener('change', () => {
	const result = /^[^\s@]+@[^\s@]+.[^\s@]+$/.test(emailInput.value);
	if (!result) emailInput.className = 'error-message';
	else emailInput.className = '';
})
...
document.forms.formTwo.addEventListener('submit', function (evt) {
	...
	if (!checkbox1 && !checkbox2 && !checkbox3) {
		document.getElementById('result21').innerHTML = 'Choose one of checkboxes';
		return
	}
});
...
document.querySelector('.b-22').onclick = function (event) {
	...
	if (!nameInput) {
		document.getElementById('result22').innerHTML = 'Enter name!';
		return
	}
};
...
const lastFormBtn = document.forms.lastForm.elements[1];
lastFormBtn.disabled = true;

document.forms.lastForm.elements[0].addEventListener('change', (e) => {
	if (e.target.checked) lastFormBtn.disabled = false;
})
//Задание 24
...
document.querySelector('.b-25').addEventListener('click', function () {
	...
	inputs.forEach(function (input) {
		input.addEventListener('focus', () => {
			input.classList.add('task-25')
		})
		input.addEventListener('blur', () => {
			input.classList.remove('task-25')
		})
	});
});
...
document.querySelector('.b-26').addEventListener('click', function (event) {
	...
	document.getElementById("result26").innerHTML = document.forms[2].elements[0].placeholder;
});
...
formTwoInputs.forEach(function (input) {
	input.addEventListener('input', function () {
		document.getElementById("result27").innerHTML = "Изменение внесено";
	});
});
...
selectFormThree.addEventListener('change', function () {
	document.getElementById("result28").innerHTML = "Опция выбрана";
});
